...
fn showSymbolTable(allocator:std.mem.Allocator, ptr:[*]u8, hdr:*c.AOEFFheader) void {
	...
	for (0..symbtabSize) |i| {
		...
		const symbloc:c_int = c.SE_LOC(symbtab[i].seSymbInfo);
		...
		const symbtype:c_int = c.SE_TYPE(symbtab[i].seSymbInfo);
		if (symbtype == c.SET_ADDR) {
			print("Address type\n", .{});
		} else if (symbtype == c.SET_ABSV) {
			print("Absolute type\n", .{});
		} else if (symbtype == c.SET_NONE) {
			print("None type\n", .{});
		} else {
			print("INVALID TYPE\n", .{});
		}
